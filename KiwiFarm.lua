-- ============================================================================
-- KiwiFarm WotLK 3.3.5 backport (C) Keoo
-- ============================================================================
...
local VANILA  = VERSION < 30000 -- Vanilla/TBC: per-char resets, 5/hour limit

local function GetExpansionLevelCompat()
	if GetExpansionLevel then
		return GetExpansionLevel()
	end
	if VERSION >= 30000 then return 2 end -- WotLK
	if VERSION >= 20000 then return 1 end -- TBC
	return 0 -- Vanilla
end

local TimerAfter = C_Timer and C_Timer.After or function(delay, func)
...
do
	...
	local levelMax = (MAX_PLAYER_LEVEL_TABLE[GetExpansionLevelCompat()] or 0)
	local levelCap = isSoD and level<=50 and 50
	isPlayerLeveling = level < (levelCap or levelMax)
end

-- default values
local RESET_MAX = 5 -- 3.3.5 WotLK: 5 instance resets per hour
local RESET_DAY = 30
...
local DEFSERVER = {
	...
	resetData = VANILA and {}, -- reset data per character for vanilla/tbc
	resets    = (not VANILA) and {count=0,countd=0}, -- reset data per server for wotlk+
	resetsd   = (not VANILA) and {}, -- reset data per server for wotlk+
}

local DEFRESET = {
	resets  = {count=0,countd=0}, -- resets per hour
	resetsd = {}, -- resets per day  (max 30, only for vanilla/tbc)
}
...
local function CreateDB()
	...
	if VANILA then -- move resets per realm to resets per char (due to blizzard hotfix) but only in vanilla/tbc
...
do
	...
	local function InitAuctionator()
		if Atr_GetAuctionPrice and (Atr_GetDisenchantValue or Atr_CalcDisenchantPrice) then -- Auctionator 3.x (WotLK/TBC/Vanilla)
			-- LibItemUpgradeInfo is not used on 3.3.5; without this fallback Atr prices never apply
			ItemUpgradeInfo = LibStub('LibItemUpgradeInfo-1.0', true) or true
			Auctionator_GetMarketPrice = function(name, itemID)
				return Atr_GetAuctionPrice(name) or (itemID and Atr_GetAuctionPrice(itemID))
			end
			Auctionator_GetDisenchantPrice = function(itemLink, class, rarity)
				...
				local itemLevel = 0
				if type(ItemUpgradeInfo) == "table" and ItemUpgradeInfo.GetUpgradedItemLevel then
					itemLevel = ItemUpgradeInfo:GetUpgradedItemLevel(itemLink) or 0
				end
				return Atr_CalcDisenchantPrice(class, rarity, itemLevel)
			end
	...
	local function GetValue(source, itemLink, itemID, name, class, rarity, vendorPrice, userPrice)
		...
		elseif source == 'vendor' then
			price = vendorPrice
		elseif source == 'Atr:DBMarket' and Auctionator_GetMarketPrice then -- Auctionator: market
			price = Auctionator_GetMarketPrice(name, itemID)
		elseif source == 'Atr:Destroy' and Auctionator_GetDisenchantPrice then -- Auctionator: disenchant
